"""Thin FastAPI gateway for the query agent (Phase 2 Step 6).
...
from app.api_schemas import HealthResponse, QueryRequest, QueryResponse
...
async def query_suburbs(body: QueryRequest) -> QueryResponse:
    ...
    if not _suburbs_dataset_loaded():
        raise HTTPException(
            status_code=503,
            detail="suburbs.json missing — run scripts/build_suburbs_dataset.py",
        )
    ...
    try:
        payload = await handle_query_v2(prompt, save_searches=body.save_audit)
