...
async def run_cases(cases: list[dict]) -> list[dict]:
    ...
    for case in cases:
        ...
        rows.append({
            **case,
            "route_intent": payload["route"].get("intent"),
            "trust_gate": payload.get("trust_gate"),
            "unsupported_field": payload["route"].get("unsupported_field"),
...
def score(rows: list[dict]) -> dict:
    ...
    for row in rows:
        ...
        if "expect_gate" in row:
            ...
            if row.get("expect_intent"):
                ok = ok and row.get("route_intent") == row["expect_intent"]
            if row.get("unsupported_field"):
                ok = ok and bool(row.get("unsupported_field"))
            if cat == "compare_ok":
